...
#' @export
#'
plot_covtrend <- function(mdl, xcov, trend_cov = NA, ci_cov = NA,  ci_col = "black", ev, ev_x = 0, rp = c(6,40), add_loess = F, loess_col = "forestgreen",
  ...
  x <- mdl$data[,xcov]
  if(missing(ev))    { ev <- mdl$ev }
  if(missing(ev_x)) { ev_x <- x[mdl$idx] }
  o <- order(x)
